...
export function LocaleProvider({ children, initialLocale }: { children: ReactNode; initialLocale?: Locale | null }) {
  ...
  async function setLocale(next: Locale) { const previous = locale; setCurrent(next); try { await apiFetch('/auth/locale', { method: 'PATCH', body: JSON.stringify({ locale: next }) }); } catch (error) { setCurrent(previous); throw error; } }
...
export function useLocale(): LocaleContextValue { const value = useContext(LocaleContext); if (!value) throw new Error('useLocale must be used within LocaleProvider'); return value; }
